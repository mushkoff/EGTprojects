...
void FileOperrands::traversing()
{
	...
	for(pugi::xml_node studentObj = students.child("students:student");studentObj;
			studentObj = studentObj.next_sibling("students:student"))
	{
		string firstName = studentObj.child("students:firstName").text().as_string();
		cout<<firstName;
		string lastName = studentObj.child("students:lastName").text().as_string();
        string gender = studentObj.child("students:gender").text().as_string();
        string birthday = studentObj.child("students:birthdate").text().as_string();
        string phoneNumber = studentObj.child("students:phone").text().as_string();
        string email = studentObj.child("students:email").text().as_string();
        string course = studentObj.child("students:course").text().as_string();
        string speciality = studentObj.child("students:specialty").text().as_string();
        int facultyNumber = studentObj.child("students:facultyNumber").text().as_int();

        Student *nStudent = new Student(firstName, lastName, gender, birthday, phoneNumber,
        		email, course, speciality, facultyNumber);
        ...
        for(pugi::xml_node examObj = studentObj.child("exams:examList").first_child();
        		examObj; examObj = examObj.next_sibling())
        {
        	//CHANGE THE NAMES!!!!!
